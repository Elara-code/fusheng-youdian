...
<script>const img=document.querySelector('#card');document.querySelector('#draw').onclick=()=>{const c=document.querySelector('#character').value;img.src='/card.svg'+(c?'?character='+c:'');img.hidden=false}</script>`, { headers: { "Content-Type": "text/html; charset=utf-8" } });
    }
    ...
    if (url.pathname === "/card.svg") {
      ...
      return new Response(`<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1200 800"><rect width="1200" height="800" fill="#191612"/><image href="${image}" x="70" y="50" width="1060" height="700" preserveAspectRatio="xMidYMid meet"/><rect x="70" y="560" width="1060" height="190" fill="#191612" opacity=".88"/><text x="110" y="625" fill="#f5ead5" font-size="34" font-family="serif">${quote}</text><text x="110" y="685" fill="#cdbb9c" font-size="22" font-family="serif">——${source}</text><text x="110" y="720" fill="#92794f" font-size="16" font-family="sans-serif">${xml(card.id)}</text></svg>`, { headers: { "Content-Type": "image/svg+xml; charset=utf-8", "Cache-Control": "no-store", "X-Card-Id": card.id, "X-Card-State": card.state } });
    }
