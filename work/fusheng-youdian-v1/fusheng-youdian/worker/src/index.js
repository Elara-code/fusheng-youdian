...
<style>body{margin:0;background:#171512;color:#f5ead5;font:16px system-ui;text-align:center}main{max-width:760px;margin:8vh auto;padding:24px}h1{font:42px Georgia;margin:0 0 8px}p{color:#b9aa91}img{display:block;width:100%;max-height:70vh;object-fit:contain;margin:24px 0;border-radius:12px;background:#2a251f}.show{animation:reveal .55s ease both}@keyframes reveal{from{opacity:0;transform:translateY(12px) scale(.98)}to{opacity:1;transform:none}}button,select{padding:12px 16px;border:1px solid #92794f;border-radius:999px;background:#d6ad68;color:#211a11;font-weight:700}select{margin-right:8px;background:#f5ead5}</style>
...
<script>const img=document.querySelector('#card');document.querySelector('#draw').onclick=()=>{const c=document.querySelector('#character').value;img.classList.remove('show');void img.offsetWidth;img.src='/card.svg?nonce='+Date.now()+(c?'&character='+c:'');img.hidden=false;img.classList.add('show')}</script>`, { headers: { "Content-Type": "text/html; charset=utf-8" } });
    }
